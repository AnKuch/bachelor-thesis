...
\draw[->] (T3.south) -- (CNode2.north);
\path (CNode2) edge [loop right,thick] node [right, midway, rectangle split,rectangle split parts=2]{%
  imitate, mint, Remove,
  \nodepart{second}
  Revoke, Move, Recycle%
  } (CNode2);
\draw[->] (CNode2.west) -- (AEP.east) node[ midway , above ] {Wait Event};
\draw[black, dotted, rounded corners] (-2,-7) rectangle (13.75,5);
\node[thick] at (-1,-7.5) {Domain 1};
\draw[black, dotted, rounded corners] (15.5,-7) rectangle (30,5);
\node[thick] at (16.5,-7.5) {Domain 2};
